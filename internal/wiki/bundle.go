...
// ModuleTreeNode describes a module in the navigation tree.
...
// BundleMeta holds metadata written alongside wiki pages.
...
// Bundle reads all .md files and module_tree.json from wikiDir and
// produces a self-contained index.html. The HTML includes embedded
// CSS, JavaScript, and all page content — viewable offline.
func Bundle(wikiDir, projectName string) (string, error) {
	// Load module tree (optional).
	var moduleTree []ModuleTreeNode
	if data, err := os.ReadFile(filepath.Join(wikiDir, "module_tree.json")); err == nil {
		_ = json.Unmarshal(data, &moduleTree)
	}

	// Load meta (optional).
	var meta *BundleMeta
	if data, err := os.ReadFile(filepath.Join(wikiDir, "meta.json")); err == nil {
		meta = &BundleMeta{}
	...
	if err != nil {
		return "", fmt.Errorf("read wiki dir: %w", err)
	}
	...
	for _, e := range entries {
		...
		content, err := os.ReadFile(filepath.Join(wikiDir, e.Name()))
		if err != nil {
			continue
		}
		slug := strings.TrimSuffix(e.Name(), ".md")
		pages[slug] = string(content)
	}

	if len(pages) == 0 {
		return "", fmt.Errorf("no .md files found in %s", wikiDir)
	}

	html := buildHTML(projectName, moduleTree, pages, meta)
	outputPath := filepath.Join(wikiDir, "index.html")
...
// buildHTML assembles the self-contained HTML page.
func buildHTML(projectName string, tree []ModuleTreeNode, pages map[string]string, meta *BundleMeta) string {
	escScript := func(s string) string {
	...
	b.WriteString("<title>")
	b.WriteString(escapeHTML(projectName))
	b.WriteString(" — Wiki</title>\n")
	b.WriteString("<script src=\"https://cdn.jsdelivr.net/npm/marked@11.0.0/marked.min.js\"></script>\n")
	...
	// Body.
	b.WriteString("<body>\n")
	b.WriteString("<button class=\"menu-toggle\" id=\"menu-toggle\" aria-label=\"Toggle menu\">&#9776;</button>\n")
	b.WriteString("<div class=\"layout\">\n")

	// Sidebar.
	b.WriteString("<nav class=\"sidebar\" id=\"sidebar\">\n")
	b.WriteString("<div class=\"sidebar-header\">\n")
	b.WriteString("<div class=\"sidebar-title\">")
	b.WriteString(bookSVG)
	b.WriteString(escapeHTML(projectName))
	b.WriteString("</div>\n")
	b.WriteString("<div class=\"sidebar-meta\" id=\"meta-info\"></div>\n")
	b.WriteString("</div>\n")
	b.WriteString("<div id=\"nav-tree\"></div>\n")
	b.WriteString("<div class=\"sidebar-footer\">Generated by Cartograph</div>\n")
	b.WriteString("</nav>\n")

	// Content.
	b.WriteString("<main class=\"content\" id=\"content\">\n")
	b.WriteString("<div class=\"empty-state\"><h2>Loading…</h2></div>\n")
	b.WriteString("</main>\n")
	b.WriteString("</div>\n")

	// Script.
	b.WriteString("<script>\n")
	b.WriteString("var PAGES = ")
	b.WriteString(escScript(string(pagesJSON)))
	b.WriteString(";\n")
	b.WriteString("var TREE = ")
	b.WriteString(escScript(string(treeJSON)))
	b.WriteString(";\n")
	b.WriteString("var META = ")
	b.WriteString(escScript(string(metaJSON)))
	b.WriteString(";\n")
	b.WriteString(jsApp)
	b.WriteString("\n</script>\n")
...
const bookSVG = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 3h6a4 4 0 014 4v14a3 3 0 00-3-3H2z"/><path d="M22 3h-6a4 4 0 00-4 4v14a3 3 0 013-3h7z"/></svg>`

//nolint:dupword // CSS var() repetitions are not duplicate words.
...
const jsApp = `(function() {
  ...
  function renderMeta() {
    ...
    var parts = [];
    if (META.generatedAt) parts.push(new Date(META.generatedAt).toLocaleDateString());
    if (META.repo) parts.push(META.repo);
    if (META.commit) parts.push(META.commit.slice(0, 8));
